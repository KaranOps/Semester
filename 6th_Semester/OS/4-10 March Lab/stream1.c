...
#include <stdio.h>
#include <stdlib.h> /* the manual for exit() function (used in this program) tells that this header file has to be
 included - "man 3 exit"*/
...
int main(int argc, char *argv[]) {
	...
	if (fp == NULL) { /* If for some reason fopen cannot "open" the file, it will return NULL) */
		printf("The file %s cannot be opened for reading!!!\n", argv[1]); /* argv[1] is the name of the file the 
		user has given as the command line argument! Eg., For example if the user has executed the program as 
		"./a.out abc.txt", then argv[1] will contain "abc.txt" */
		exit(1); /* what does it do? Read manual by "man 3 exit" */
	}
	...
	while (!feof(fp)) { /* the stream fp has NOT reached "End Of File" - eof. If eof - the loop ends */
		...
		ch = fgetc(fp);
		if ( (ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z')) { /* ch is an English letter - either small or 
			capital */
			countE = countE + 1;
		}
