...
int main(int argc, char *argv[])
{
        ...
        if (fp == NULL)
        {                                                                         /* If for some reason fopen cannot "open" the file, it will return NULL) */
        ...
        while (!feof(fp))
        { /* the stream fp has NOT reached "End Of File" - eof. If eof - the loop ends */
                ...
                if (ch == '.')
                {                 /* this is the end of a new sentence */
                ...
                { /* the new character is non-white */
                        if (isPreviousWhite == true)
                        { /* the previous character was white and the new character is non-white - it is a new word then */
                                countW++;
