...
int initFile(char *filename, int blsize, int blno ){
             int  ofd = open("inmarch.txt", O_CREAT|O_RDWR, 0700);
              if(ofd == -1) {
              ...
              blockzero.ubn = 0;
              blockzero.fbn = 4096;
              memset(blockzero.ub, '0', sizeof(blockzero.ub)); // initialize all blocks whith 0;
              ...
              for(int i = 0; i < blno; i++){
                if(write(ofd, &buffer, sizeof(Block)) != sizeof(Block)){    perror("Block write failed");
                        close(ofd);
                        return -1;
                       }
                     }
       ...
       int get_freeblock(char *filename){
               int  fd = open("inmarch.txt", O_CREAT|O_RDWR, 0700);
               if(fd == -1){
...
int main() {
        int fd;
         fd = initFile("inmarch.txt", 4096, 2048);
        if(fd == 0)
        ...
        for(int i = 0; i < 1; i++){
              int freeB = get_freeblock("inmarch.txt");
              if(freeB == -1)
       ...
       for(int i = 0; i < 2; i++){
             ...
             scanf("%d",&blockNo);
             int freedBlock = free_block("inmarch.txt",blockNo);
             if(freedBlock == -1)
     ...
     int result = check_fs("inmarch.txt");
         if (result == 0) {
