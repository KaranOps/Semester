...
The 1st 8 blocks (8 x 4096 bytes) of dd1 will store the metadata (name and size) of the linux files contained in dd1.
Remaining 2048 blocks of dd1 are the data blocks of the contained files. The files contained in dd1 satisfy the following constraints.

Size of each file can at most 4096 bytes requiring 1 block of dd1.
Name of each can have 12 characters at the most.

To store the metedata of every linux file contained in dd1, 16 bytes (12 for the name and 4 for the size).
Since each file needs one block, at most 2048 files can be stored in dd1. In the 1st 8 blocks too, metadata of 2048 files
can be stored,  that is, (8 x 4096) / 16 = 2048,
...
int main(int argc, char *argv[])
{
	...
	if (strcmp(basename, "mymkfs") == 0)
	{
		...
		if (flag != 0)
		{ /* mymkfs() has failed for some reason */
	...
	else if (strcmp(basename, "mycopyTo") == 0)
	{
		...
		if (flag != 0)
		{ /* mycopyTo() has failed for some reason */
			fprintf(stderr, "%s Failed!\n", argv[0]);
		}
	}
	else if (strcmp(basename, "mycopyFrom") == 0)
	{
		...
		if (flag != 0)
		{ /* mycopyFrom() has failed for some reason */
			fprintf(stderr, "%s Failed!\n", argv[0]);
		}
	}
	else if (strcmp(basename, "myrm") == 0)
	{
		...
		flag = myrm(argv[1]);
		if (flag != 0)
		{ /* myrm() has failed for some reason */
			fprintf(stderr, "%s Failed!\n", argv[0]);
		}
	}
	else
...
int mycopyTo(char *fname, char *mfname)
{
	...
	hole = -1;
	for (i = 0; i < NOFILES; i++)
	{ /* check all the myfile names in 16 byte myfile descriptors.*/
		if (sbuf[i * 16] == 0)
		{
			hole = i;
		}
